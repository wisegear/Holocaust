...
<div class="my-5 text-center">
	...
	{{ method_field('PUT') }}
		<div class="space-x-4">  
			...
			<button type="submit" class="border rounded-md py-1 px-2 bg-red-300 text-sm" name="closeTicket" value="true">Close Ticket</button> 
			@elseif ( $ticket->status == "Closed" || $ticket->status == "In Progress")
			<button type="submit" class="border rounded-md py-1 px-2 bg-green-300 text-sm" name="openTicket" value="true">Open Ticket</button> 
			@endif
			@can('Admin')
			<button type="submit" class="border rounded-md py-1 px-2 bg-indigo-300 text-sm" name="inProgress" value="true">In Progress</button>
			<button type="submit" class="border rounded-md py-1 px-2 bg-yellow-300 text-sm" name="AwaitingReply" value="true">Awaiting Reply</button>
			@endcan
		</div>  
	</form>	
</div>
	...
	{{ method_field('PUT') }}     
		<div class="text-center">
			<div class="text-red-500">{{ $errors->has('comment') ? 'You need to tell us something before replying :)' : '' }}</div>
				<textarea class="w-full border rounded-md" name="comment" id="text" placeholder="Reply here."></textarea>
		</div>  
		<button type="submit" class="border rounded-md py-1 px-2 bg-green-300 text-sm my-2" style="">Add Reply</button> 
	</form>			
</div>
