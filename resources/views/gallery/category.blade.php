         ...
         @foreach ($gallery_albums as $gallery_album)
          <div class="relative">
            ...
            <a href="/gallery/album/{{$gallery_album->slug}}">
              <img src="{{ $gallery_path }}/{{ strtolower($gallery_album->GalleryCategories->name)}}/{{ strtolower($gallery_album->name) }}/thumb-{{$gallery_album->galleryImages->random()->image}}" alt="" class="shadow-md">
                <div class="absolute right-0 top-5">
                  <span class="py-1 px-4 border-t border-b border-l bg-gray-100 text-xs shadow-md">{{ $gallery_album->name }}</span>
                </div>
            </a>
            @endif
          </div>
